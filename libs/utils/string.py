...
def glob_ranges(spec: str | T.Iterable[str]) -> T.Iterable[str]:
  """Returns all expanded glob patterns."""
...
def filter_by_glob_range(spec: str | T.Iterable[str], items: T.Iterable[str]) -> T.Iterable[str]:
  """Returns all `item` strings that match `spec`, e.g. 'b_foof_1' matches '[a-c]_foo*_[1-3]'."""
  return (
    item for item in items
    if any(fnmatch.fnmatchcase(item, pattern) for pattern in glob_ranges(spec))
  )
...
def select_by_glob_range(spec: T.Mapping[str, T.Any], key: str, default: T.Any = _EMPTY) -> T.Any:
  """Selects a value from `spec` based on first glob pattern that matches `key`."""
  for pattern, value in spec.items():
    if fnmatch.fnmatchcase(key, pattern):
      return value
  if default is not _EMPTY:
    return default
  raise KeyError(f'No match for key: {key}')


def timestamp(format: str = '%Y-%m%d-%H%M%S', tz: datetime.tzinfo | None = None) -> str:
  """Returns current timestamp string in specified format."""
...
def hruid(n: int = 2, sep: str = '-'):
  """Generate human-readable unique identifier, e.g. 'adorable-aardvarks'."""
  assert 1 <= n <= 5
  parts = []
